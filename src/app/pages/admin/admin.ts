...
export class Admin {
  ...
  protected readonly users = signal<AdminUser[]>([{ id: 1, name: this.auth.session()?.name ?? 'Administración', email: this.auth.session()?.email ?? 'admin@organizacion.org', status: 'Activo' }]);
  ...
  protected profileEmail = this.auth.session()?.email ?? 'admin@organizacion.org';
  ...
  protected saveProfile(): void { if (!this.profileName || !this.profileEmail.includes('@')) return; this.auth.updateProfile(this.profileName, this.profileEmail); this.users.update(items => items.map((user, index) => index === 0 ? { ...user, name: this.profileName, email: this.profileEmail } : user)); this.profileMessage.set('Perfil actualizado correctamente.'); }
