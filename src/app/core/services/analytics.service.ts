...
export class AnalyticsService {
  private readonly visitKey = 'site-visit';
  private readonly analyticsKey = 'site-analytics';
  readonly data = signal<AnalyticsData>(this.read());

  constructor(router: Router) {
    if (!sessionStorage.getItem(this.visitKey)) {
      sessionStorage.setItem(this.visitKey, '1');
  ...
  reset(): void { const empty = { visits: 0, pageViews: 0, clicks: 0, pages: {} }; localStorage.setItem(this.analyticsKey, JSON.stringify(empty)); this.data.set(empty); }
  private update(change: (data: AnalyticsData) => AnalyticsData): void { const next = change(this.data()); localStorage.setItem(this.analyticsKey, JSON.stringify(next)); this.data.set(next); }
  private read(): AnalyticsData { try { return JSON.parse(localStorage.getItem(this.analyticsKey) ?? '') as AnalyticsData; } catch { return { visits: 0, pageViews: 0, clicks: 0, pages: {} }; } }
}
